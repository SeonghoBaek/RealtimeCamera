...
def decoder_network(latent, anchor_layer=None, activation='swish', scope='g_decoder_network', bn_phaze=False):
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        ...
        l = add_residual_block(l, filter_dims=[3, 3, g_dense_block_depth], num_layers=2,
                                     act_func=act_func, bn_phaze=bn_phaze, use_residual=True, scope='block_1')

        l = add_residual_block(l, filter_dims=[3, 3, g_dense_block_depth], num_layers=2,
                               act_func=act_func, bn_phaze=bn_phaze, use_residual=True, scope='block_1_1')
        ...
        l = add_residual_block(l, filter_dims=[3, 3, g_dense_block_depth], num_layers=2,
                                     act_func=act_func, bn_phaze=bn_phaze, use_residual=True, scope='block_2')

        l = add_residual_block(l, filter_dims=[3, 3, g_dense_block_depth], num_layers=2,
                               act_func=act_func, bn_phaze=bn_phaze, use_residual=True, scope='block_2_1')
        ...
        l = add_residual_block(l, filter_dims=[3, 3, g_dense_block_depth], num_layers=2,
                                     act_func=act_func, bn_phaze=bn_phaze, use_residual=True, scope='block_3')

        l = add_residual_block(l, filter_dims=[3, 3, g_dense_block_depth], num_layers=2,
                               act_func=act_func, bn_phaze=bn_phaze, use_residual=True, scope='block_3_1')
        ...
        l = add_residual_block(l, filter_dims=[3, 3, g_dense_block_depth], num_layers=2,
                                     act_func=act_func, bn_phaze=bn_phaze, use_residual=True, scope='block_4')

        l = add_residual_block(l, filter_dims=[3, 3, g_dense_block_depth], num_layers=2,
                               act_func=act_func, bn_phaze=bn_phaze, use_residual=True, scope='block_4_1')

        l = add_residual_block(l, filter_dims=[3, 3, g_dense_block_depth], num_layers=2,
                               act_func=act_func, bn_phaze=bn_phaze, use_residual=True, scope='block_4_2')
...
def add_residual_block(in_layer, filter_dims, num_layers, act_func=tf.nn.relu, bn_phaze=False, use_residual=True, scope='residual_block'):
...
def encoder_network(x, activation='relu', scope='encoder_network', reuse=False, bn_phaze=False, keep_prob=0.5):
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        ...
        l = layers.conv(x, scope='conv1', filter_dims=[3, 3, g_dense_block_depth], stride_dims=[1, 1],
                        dilation=[1, 4, 4, 1], non_linear_fn=None, bias=False)
...
def train(model_path):
    ...
    p_feature, p_logit, p_prob = latent_discriminator(latent_real, activation='relu', scope='discriminator', bn_phaze=bn_train)
    n_feature, n_logit, n_prob = latent_discriminator(latent_fake, activation='relu', scope='discriminator', bn_phaze=bn_train)
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        ...
        for i in range(num_epoch):
            ...
            for start, end in training_batch:
                ...
                #sess.run(
                #    [f_optimizer],
                #    feed_dict={X: trX[start:end], Y: trY[start:end], ANCHOR: anchor[0],
                #               bn_train: True,
                #               keep_prob: 0.5})
...
def test(model_path, test_image_dir):
    ...
    with tf.device('/device:CPU:0'):
        ...
        for idx, labelname in enumerate(test_image_dir_list):
            ...
            if os.path.isdir(os.path.join(test_output_dir, labelname).replace("\\", "/")) is False:
                continue
