...
#ifdef REDIS
    draw_and_send_detections(gRedisContext, det, l.w*l.h*l.n, camera_thresh, boxes, probs, camera_names, NULL /*camera_alphabet*/, gFrameNum);
#else
...
void detect_camera(char *cfgfile, char *weightfile, float thresh, int cam_index, const char *filename, char **names, int classes, int frame_skip, char *prefix)
{
    fprintf(stderr, "Camera needs OpenCV for webcam images.\n");
}
#endif
